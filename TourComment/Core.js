	...
	return new Promise((resolve) => {
		...
		var tours = (configuration) => {
			return new Promise((resolve_tours) => {
				var query = db.query("SELECT * FROM `tours` WHERE end_date = '"+end_date+"'", async (err, result) => {
					if (err) {
						...
						if (result.length > 0) {
							...
							logs_arr.push("["+sekarang+"] --- SELECT * FROM `tours` WHERE end_date = '"+end_date+"'");
		...
		var trx_booking = (tour_id, configuration) => {
			return new Promise((resolve_booking) => {
				var query = db.query("SELECT * FROM `trx_booking` WHERE (dp_booking_code LIKE '%-1' OR dp_booking_code = '') AND status = 'success' AND transaction_id IN ("+tour_id+")", async (err, result) => {
					if (err) {
		...
		var trx_booking_contact = (booking_id, configuration) => {
			return new Promise((resolve_booking_contact) => {
				var query = db.query("SELECT bc.*, t.id AS tour_id, t.title as judul_trip, t.guide_tour_program_id FROM `trx_booking_contact` AS bc JOIN `trx_booking` AS b ON bc.booking_id = b.id JOIN `tours` AS t ON b.transaction_id = t.id WHERE bc.booking_id IN ("+booking_id+") AND bc.email != '' GROUP BY bc.email", (err, result) => {
					if (err) {
						...
						if (result.length > 0) {
							logs_arr.push("["+sekarang+"] --- SELECT bc.*, t.id AS tour_id, t.title as judul_trip, t.guide_tour_program_id FROM `trx_booking_contact` AS bc JOIN `trx_booking` AS b ON bc.booking_id = b.id JOIN `tours` AS t ON b.transaction_id = t.id WHERE bc.booking_id IN ("+booking_id+") AND bc.email != '' || NULL GROUP BY bc.email");

							var insert_sendmail = async () => {
								var count = result.length;
								for (var i = 0; i < result.length; i++) {
									var [bool, logs_arr] = await insert_data(result[i]);
									if (bool === true) {
										var [logs_arr] = await send_mail(Object.assign(result[i], configuration));
									}
									count--;
									if (count === 0) {
										resolve(logs_arr);
		...
		var insert_data = (data) => {
			return new Promise((resolve_insert) => {
				var sql = "INSERT INTO `tour_comment_notifications` (`code`, `email`, `user_id`, `trx_booking_id`, `guide_tour_program_id`, `tour_id`) VALUES ('"+generate_code_token(data.email+data.booking_id)+"', '"+data.email+"', "+data.user_id+", "+data.booking_id+", "+data.guide_tour_program_id+", "+data.tour_id+")";
				db.query(sql, (err) => {
					if (err) {
		...
		var send_mail = (data) => {
			return new Promise((resolve_send) => {
				ejs.renderFile(__dirname+'/../templates/'+data.keydata+'.ejs', {data: data, token: generate_code_token(data.email+data.booking_id), config_url: config.config_url}, (err, html) => {
					if (err) {
