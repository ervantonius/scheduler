		...
		db.query(query2, async (err, result) => {
			if (err) {
				...
				for (var i = 0; i < result.length; i++) {
					...
					var [photo] = await get_photo(result[i].transaction_id);
					if (contact) {
						result[i].total_price = total_price;
						result[i].photo = (photo) ? config.asset_phototour + result[i].product_id + "/square_" + result[i].photo : config.asset_phototour + "images/og.png";
...
var get_photo = (tour_id) => {
	return new Promise((resolve) => {
		var query3 = "SELECT t.id, p.source FROM tours AS t LEFT JOIN tour_photo AS p ON p.tour_id = t.id WHERE t.id = "+tour_id+" AND p.primary_photo = '1' LIMIT 1";
		db.query(query3, async (err, result) => {
			if (err) {
				logger.error('Query SELECT `tour_photo` WHERE tour_id = '+tour_id+' error: ', err);
				resolve([false]);
			}
			else if (result.length === 0) {
				logger.warn("No data from `tour_photo` WHERE tour_id = "+tour_id);
				resolve([false]);
			}
			else {
				resolve([result[0].source]);
			}
		});
	});
}
...
var get_contact = (booking_code) => {
	return new Promise((resolve) => {
		db.query("SELECT id FROM trx_booking WHERE payment_method = 'saving' AND dp_booking_code = '"+booking_code+"-1'", async (err, result) => {
			if (err) {
				logger.error('Query SELECT id `trx_booking` = '+booking_code+' error: ', err);
				resolve([false]);
			}
			else if (result.length === 0) {
		...
		var info_contact = (booking_id) => {
			db.query("SELECT email, title AS title_name, first_name, last_name, phone_number FROM trx_booking_contact WHERE primary_contact = '1' AND booking_id = "+booking_id, (err, result) => {
				if (err) {
					logger.error('Query SELECT `trx_booking_contact` = '+booking_id+' error: ', err);
					resolve([false]);
				}
				else if (result.length === 0) {
