	...
	return new Promise((resolve) => {
		...
		var tours = (configuration) => {
			return new Promise((resolve_tours) => {
				var query = db.query("SELECT * FROM `tours` WHERE end_date = '"+end_date+"' AND status IN ('confirm', 'close')", async (err, result) => {
					if (err) {
		...
		var insert_data = (data, token) => {
			return new Promise((resolve_insert) => {
				var sql = "INSERT INTO `tour_comment_notifications` (`code`, `email`, `user_id`, `trx_booking_id`, `guide_tour_program_id`, `tour_id`) VALUES (?, ?, ?, ?, ?, ?)";
				db.query(sql, [token, data.email, data.user_id, data.booking_id, data.guide_tour_program_id, data.tour_id] (err) => {
					if (err) {
