	...
	return new Promise((resolve) => {
		...
		var trx_booking_contact = (booking_id, configuration) => {
			return new Promise((resolve_booking_contact) => {
				var query = db.query("SELECT bc.*, t.id AS tour_id, t.title as judul_trip, t.guide_tour_program_id, n.row_status FROM `trx_booking_contact` AS bc JOIN `trx_booking` AS b ON bc.booking_id = b.id JOIN `tours` AS t ON b.transaction_id = t.id LEFT JOIN `tour_comment_notifications` AS n ON bc.booking_id = n.trx_booking_id AND bc.user_id = n.user_id AND t.id = n.tour_id AND t.guide_tour_program_id = n.guide_tour_program_id WHERE bc.booking_id IN ("+booking_id+") AND bc.email != '' GROUP BY bc.email, tour_id", (err, result) => {
					if (err) {
						...
						if (result.length > 0) {
							logs_arr.push("["+sekarang+"] --- SELECT bc.*, t.id AS tour_id, t.title as judul_trip, t.guide_tour_program_id, n.row_status FROM `trx_booking_contact` AS bc JOIN `trx_booking` AS b ON bc.booking_id = b.id JOIN `tours` AS t ON b.transaction_id = t.id LEFT JOIN `tour_comment_notifications` AS n ON bc.booking_id = n.trx_booking_id AND bc.user_id = n.user_id AND t.id = n.tour_id AND t.guide_tour_program_id = n.guide_tour_program_id WHERE bc.booking_id IN ("+booking_id+") AND bc.email != '' GROUP BY bc.email, tour_id");

							var insert_sendmail = async () => {
								var count = result.length;
								for (var i = 0; i < result.length; i++) {
									var token = generate_code_token(result[i].email+result[i].booking_id);
									if (result[i].row_status === null) {
										var [status, response, logs_arr] = await send_mail(Object.assign(result[i], configuration), token);
										var [logs_arr] = await insert_data(result[i], token, response, status);
									} else if (result[i].row_status === 1) {
										var [status, response, logs_arr] = await send_mail(Object.assign(result[i], configuration), token);
										var [logs_arr] = await update_data(token, response, status);
									}
									count--;
									if (count === 0) {
										resolve(logs_arr);
		...
		var insert_data = (data, token, response, row_status) => {
			return new Promise((resolve_insert) => {
				var sql = "INSERT INTO `tour_comment_notifications` (`code`, `email`, `user_id`, `trx_booking_id`, `guide_tour_program_id`, `tour_id`, `created_by`, `response`, `row_status`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
				db.query(sql, [token, data.email, data.user_id, data.booking_id, data.guide_tour_program_id, data.tour_id, 'momo-scheduler', JSON.stringify(response), row_status], (err) => {
					if (err) {
		...
		var update_data = (token, response, row_status) => {
			return new Promise((resolve_insert) => {
				var sql = "UPDATE `tour_comment_notifications` SET `modified_by` = ?, `response` = ?, `row_status` = ? WHERE `code` = ?";
				db.query(sql, ['momo-scheduler', JSON.stringify(response), row_status, token], (err) => {
					if (err) {
						logger.error('Query Insert error: ', err);
						logs_arr.push('['+sekarang+'] --- Query Insert error: ', err);
						resolve_insert([false, logs_arr]);
						...
						logger.info(sql);
						logger.info('Update data berhasil');
						logs_arr.push("["+sekarang+"] --- "+sql);
						resolve_insert([true, logs_arr]);
					}
				});
