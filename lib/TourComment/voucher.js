...
var insert_data = (data) => {
	return new Promise((resolve) => {
		...
		var insert = (code_voucher) => {
			...
			var query1 = db.query(sql, [code_voucher, data.trans_type, data.product_id, data.value_type, data.value, data.min_transaction, data.min_pax, data.max_discount, data.reuse, data.maxlimit, data.email, sekarang, moment().add(data.expired_days, 'days').format('YYYY-MM-DD HH:mm:ss'), data.description, sekarang], (err, result) => {
				if (err) {
					logger.error('Query Insert `t_mst_coupon` error: ', err);
					resolve([false]);
				}
				else {
					var query2 = db.query("INSERT INTO `tour_comment_coupons` (created_by, modified_by, comment_id, coupon_id) VALUES (?, ?, ?, ?)", [data.created_by, data.modified_by, data.id, result.insertId], (err) => {
						if (err) {
