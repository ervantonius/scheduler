	...
	return new Promise((resolve) => {
		var query = db.query("SELECT * FROM `tour_comment_coupons` ORDER BY id DESC LIMIT 1", async (err, result) => {
			if (err) {
				...
				await tour_comment_null(coupon_template);
			}
			else {
...
var tour_comment_null = (coupon_template) => {
	return new Promise((resolve) => {
		var data = [];
		var query = db.query("SELECT c.id, c.user_id, c.email, c.tour_id, cu.first_name, cu.last_name, ct.title FROM `tour_comments` AS c JOIN `t_mst_users` AS cu ON cu.id_user = c.user_id JOIN `tours` AS ct ON ct.id = c.tour_id WHERE c.email NOT IN ('0', '')", (err, result) => {
			if (err) {
				logger.error('Query Select `tour_comments` error: ', err);
				resolve(false);
			}
			else if (result.length === 0) {
